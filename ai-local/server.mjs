...
app.get("/health", (_req, res) => {
  res.json({ ok: true, model: OLLAMA_MODEL, port: PORT });
});
...
app.post("/ai/session/start", async (req, res) => {
  try {
    res.json(await startSessionCore(req.body || {}));
  } catch (err) {
    const status = Number(err?.status || 500);
    res.status(status).json({ error: String(err?.message || err), ...(err?.extra || {}) });
  }
});

app.post("/ai/session/message", async (req, res) => {
  try {
    res.json(await messageSessionCore(req.body || {}));
  } catch (err) {
    const status = Number(err?.status || 500);
    res.status(status).json({ error: String(err?.message || err), ...(err?.extra || {}) });
  }
...
app.post("/ai/session/finalize", async (req, res) => {
  try {
    res.json(await finalizeSessionCore(req.body || {}));
  } catch (err) {
    const status = Number(err?.status || 500);
    res.status(status).json({ error: String(err?.message || err), ...(err?.extra || {}) });
  }
...
export function startServer(port = PORT) {
  return app.listen(port, () => {
    console.log(`AI local API escuchando en http://127.0.0.1:${port}`);
  });
}
...
if (isDirectRun && process.env.DENUPOL_NO_LISTEN !== "1") {
  startServer(PORT);
}
